...
func spawn_random_objects(count):
	...
	while  spawned_count < count and attempts < 100:
		var position = get_random_position()		
		spawn_objects(Global.spawnable_objects[randi() % Global.spawnable_objects.size()], position)
		spawned_count += 1
		attempts += 1
		
func spawn_objects(data, position):
	var object_scene = data.path
	var object_instance = object_scene.instantiate()
	object_instance.global_position = position
	objectsNode.add_child(object_instance)
